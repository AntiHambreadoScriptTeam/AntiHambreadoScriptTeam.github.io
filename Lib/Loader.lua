--[[
	...
	Anti-Hambreado Interface Suite
	by FeliciaXxxTop & TheRealBanHammer

	FeliciaXxxTop | Designing + Programming
	TheRealBanHammer   | Programming

]]

if debugX then
	warn('Initialising Anti-Hambreado')
end
...
if _getgenv then
	local ok, result = pcall(function() return _getgenv().DISABLE_AHST_REQUESTS end)
	if ok and result then requestsDisabled = true end
	local ok2, result2 = pcall(function() return _getgenv().AHST_ASSET_ID end)
	if ok2 and type(result2) == "number" then customAssetId = result2 end
	local ok3, result3 = pcall(function() return _getgenv().AHST_SECURE end)
...
local function secureNotify(wType, title, content)
	...
	task.spawn(function()
		while not AHSTLib or not AHSTLib.Notify do task.wait(0.5) end
		AHSTLib:Notify({
			Title = title,
			Content = content,
			Duration = 8,
...
local RayfieldFolder = "AHST Lib"
local ConfigurationFolder = RayfieldFolder.."/Configurations"
local ConfigurationExtension = ".ahst"
local settingsTable = {
	General = {
		
		ahstOpen = {Type = 'bind', Value = 'K', Name = 'AHST Keybind'},
...
local function callSafely(func, ...)
	if func then
		...
		if not success then
			warn("AHST Lib | Function failed with error: ", result)
			return false
		else
			return result
		end
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		...
		if useStudio then
			file = [[
	{"General":{"ahstOpen":{"Value":"K","Type":"bind","Name":"AHST Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"AHST Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
]]
		end
		...
		if next(file) ~= nil then
			...
			for settingName, settingValue in overriddenSettings do
				...
				assert(#split == 2, "AHST Lib | Invalid overridden setting name: " .. settingName)
	...
	if not success then 
		if writefile then
			warn('AHST Lib had an issue accessing configuration saving capability.')
		end
	end
end
...
if not useStudio then
	task.spawn(function()
		pcall(game.HttpGet, game, "https://www.sentivel.com/api/heartbeat/81074364b461f8da81bad6fdc363c3b927f884d6fc28d806a15ee50ca1e68c78")
	end)
end
...
repeat
	...
	if not warned then
		warn('AHST Lib | Build Mismatch')
		print('AHST Lib may encounter issues as you are running an incompatible interface version ('.. ((Rayfield:FindFirstChild('Build') and Rayfield.Build.Value) or 'No Build') ..').\n\nThis version of AHST Lib is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end
...
if secureMode and not customAssetId then
	secureNotify("default_asset", "Secure Mode", "You are using the default AHST asset ID. Set AHST_ASSET_ID to a custom upload to avoid detection.")
end
...
do
	...
	local assetFiles = {
		["111263549366178"] = "https://antihambreadoscriptteam.github.io/Foto/Lib/Anti-Girl-Lib.jpg",
		["AHST_Background"] = "https://antihambreadoscriptteam.github.io/Foto/Lib/Fondo-Lib.png",
		...
		local ok, err = pcall(function()
			...
			for id, _ in assetFiles do
				...
				if success then
					...
					warn("AHST Lib | Failed to load custom asset: "..tostring(id).." - "..tostring(asset))
				end
			end
		end)

		if not ok then
			warn("AHST Lib | Failed to load custom assets: "..tostring(err))
			secureNotify("asset_load_fail", "AHST Lib", "Failed to load custom assets. UI images may not display correctly.")
		end
	else
		secureNotify("no_getcustomasset", "AHST Lib", "Your executor does not support getcustomasset. Some UI images may not render correctly.")
	end
...
if not Main:FindFirstChild("AHST_Background") then
	local bgImage = Instance.new("ImageLabel")
	bgImage.Name = "AHST_Background"
	bgImage.Size = UDim2.new(1, 0, 1, 0)
	bgImage.BackgroundTransparency = 1
	bgImage.ZIndex = 0
	bgImage.Image = customAssets["AHST_Background"] or ""
	local mainCorner = Main:FindFirstChildOfClass("UICorner")
	if mainCorner then
		local clonedCorner = mainCorner:Clone()
		clonedCorner.Parent = bgImage
	end
	bgImage.Parent = Main
end
...
local function getAssetUri(id)
	...
	elseif type(id) == "string" and not Icons then
		warn("AHST Lib | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("AHST Lib | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('AHST Lib had an issue decoding the configuration file, please try delete the file and reopen AHST Lib.') return end

	
	for FlagName, Flag in pairs(AHSTLib.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("AHST Lib | Unable to find '"..FlagName.. "' in the save file.")
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(AHSTLib.Flags) do
...
function AHSTLib:Notify(data) 
	task.spawn(function()
		...
		if data.Actions then
			warn('AHST Lib | Not seeing your actions in notifications?')
...
local function Hide(notify)
	...
	if notify then
		if useMobilePrompt then 
			AHSTLib:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show'.", Duration = 7, Image = 4400697855})
		else
			AHSTLib:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping " .. tostring(getSetting("General", "ahstOpen")) .. ".", Duration = 7, Image = 4400697855})
		end
	end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('AHST Settings', 0, true)

	if TabList['AHST Settings'] then
		TabList['AHST Settings'].LayoutOrder = 1000
	end

	if Elements['AHST Settings'] then
		Elements['AHST Settings'].LayoutOrder = 1000
	end
...
function AHSTLib:CreateWindow(Settings)
	if Rayfield:FindFirstChild('Loading') then
		if getgenv and not getgenv().ahstCached then
	...
	if getgenv then getgenv().ahstCached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				AHSTLib:Notify({Title = 'Build Mismatch', Content = 'AHST Lib may encounter issues as you are running an incompatible interface version ('.. ((Rayfield:FindFirstChild('Build') and Rayfield.Build.Value) or 'No Build') ..').\n\nThis version of AHST Lib is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "AHST Lib"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "AHST Interface Suite" then
		LoadingFrame.Version.Text = "AHST UI"
	end
	...
	if not Settings.DisableRayfieldPrompts then
		task.spawn(function()
			while not ahstDestroyed do
				task.wait(math.random(180, 600))
				if ahstDestroyed then break end
				AHSTLib:Notify({
					Title = "AHST Interface",
	...
	if Settings.Discord and Settings.Discord.Enabled and not useStudio and not secureMode then
		...
		if not callSafely(isfile, RayfieldFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			...
			if Settings.Discord.RememberJoins then 
				callSafely(writefile, RayfieldFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"AHST RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("AHST Lib | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with AHST specific development.')
				end
			end
		end
		...
		if not Passthrough and secureMode then
			warn("AHST Lib | Secure Mode: Key system requires a valid saved key. The key UI cannot be shown as it requires loading detectable assets.")
			Rayfield.Enabled = false
			return AHSTLib
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then
					...
					if Settings.KeySettings.SaveKey then
						...
						AHSTLib:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	if Settings.KeySystem then
		...
		if ahstDestroyed then return end
	end
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("AHST Lib | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with AHST specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) 
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= AHSTLib.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					AHSTLib.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("AHST Lib | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with AHST specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					AHSTLib.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("AHST Lib | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with AHST specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					AHSTLib.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then 
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("AHST Lib | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with AHST specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					AHSTLib.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= AHSTLib.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("AHST Lib | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with AHST specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						AHSTLib.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
			...
			Slider.Main.Progress.Size = UDim2.new(initPercentage, 0, 1, 0)
			...
			local function UpdateSlider()
				...
				local SizeX = Slider.Main.AbsoluteSize.X
				local PositionX = Slider.Main.AbsolutePosition.X
				local Percentage = math.clamp((Location - PositionX) / SizeX, 0, 1)
				local NewValue = SliderSettings.Range[1] + Percentage * (SliderSettings.Range[2] - SliderSettings.Range[1])
				...
				NewValue = tonumber(string.format("%.7g", NewValue))

				TweenService:Create(Slider.Main.Progress, TweenInfo.new(0.15, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {Size = UDim2.new(Percentage, 0, 1, 0)}):Play()
				...
				if SliderSettings.CurrentValue ~= NewValue then
					...
					if not Success then
						...
						print("AHST Lib | " .. SliderSettings.Name .. " Callback Error " .. tostring(Response))
						task.wait(0.5)
						Slider.Title.Text = SliderSettings.Name
						TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			function SliderSettings:Set(NewVal)
				...
				local Percentage = (NewVal - SliderSettings.Range[1]) / (SliderSettings.Range[2] - SliderSettings.Range[1])

				TweenService:Create(Slider.Main.Progress, TweenInfo.new(0.45, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {Size = UDim2.new(Percentage, 0, 1, 0)}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					AHSTLib.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			AHSTLib:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			AHSTLib:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('AHST Lib had an issue creating settings.') end
...
function AHSTLib:SetVisibility(visibility)
	setVisibility(visibility, false)
end

function AHSTLib:IsVisible()
	return not Hidden
end

local hideHotkeyConnection 
function AHSTLib:Destroy()
	ahstDestroyed = true
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['AHST Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[getSetting("General", "ahstOpen")]) and not processed then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function AHSTLib:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				AHSTLib:Notify({Title = "AHST Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			AHSTLib:Notify({Title = "AHST Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('AHST Configurations Error | '..tostring(result))
			AHSTLib:Notify({Title = "AHST Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
